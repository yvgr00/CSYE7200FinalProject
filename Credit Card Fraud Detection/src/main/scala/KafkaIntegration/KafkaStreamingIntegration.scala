...
object KafkaStreamingIntegration {
  ...
  df2.printSchema()
    ...
    val inputcols = df2.columns.filter(_ != "Class")
    val assembler = new VectorAssembler()
      .setInputCols(featureCols)
      .setOutputCol("assembledFeatures")
      .setHandleInvalid("keep")

    val assembledRows = assembler.transform(df2)




  val model = PipelineModel.load("D:/BIGDATASCALA/Assignments/CSYE7200/Credit Card Fraud Detection/src/main/scala/SparkMLModel/RFModel")

    val predictions = model.transform(assembledRows)
   ...
   val query = finalPredictions
    ...
    // .option("path", "D:\\Results\\") .option("checkpointLocation", "D:\\DATASCIENCE\\").start()
